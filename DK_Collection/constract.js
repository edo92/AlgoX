class Constract {
    constructor() {
        this.constractFight = async data => {
            let imgData = this.filterData(await data);
            ...
            await imgData.map((item, index) => {
                ...
                if ((minX > 950 && minX < 1275) && (maxX > 1170 && maxX < 1306) && (minY > 1244 && minY < 1310) && (maxY > 1245 && maxY < 1811)
                    ...
                    info.fighter1.name = info.fighter1.name + `${info.fighter1.name ? ' ' : ''}` + descript;
                }
                if ((minX > 1520 && minX < 1655) && (maxX > 1700 && maxX < 1885) && (minY > 1244 && minY < 1350) && (maxY > 1240 && maxY < 1345)
                ) {
                    info.fighter2.name = info.fighter2.name + `${info.fighter2.name ? ' ' : ''}` + descript;
                };
